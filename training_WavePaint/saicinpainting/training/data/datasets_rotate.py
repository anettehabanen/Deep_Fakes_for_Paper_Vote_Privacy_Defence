...
class InpaintingTrainDataset(Dataset):
    def __init__(self, indir, mask_generator, transform):
        ...
        self.mask_filenames = sorted(list(glob.glob(os.path.join(indir, '**', '*mask*.png'), recursive=True)))
        self.input_filenames = sorted(list(glob.glob(os.path.join(indir, '**', '*input*.png'), recursive=True)))
        self.output_filenames = sorted(list(glob.glob(os.path.join(indir, '**', '*output*.png'), recursive=True)))
        #self.in_files = [fname.rsplit('_mask', 1)[0] + '.png' for fname in self.mask_filenames]
    ...
    def __getitem__(self, item):
        input_path = self.input_filenames[item]
        input_ = cv2.cvtColor(cv2.imread(input_path), cv2.COLOR_BGR2RGB)
        #input_ = self.transform(image=input_)['image']
        input_ = np.transpose(input_, (2, 0, 1))

        output_path = self.output_filenames[item]
        output_ = cv2.cvtColor(cv2.imread(output_path), cv2.COLOR_BGR2RGB)
        #output_ = self.transform(image=output_)['image']
        output_ = np.transpose(output_, (2, 0, 1))
        
        mask_path = self.mask_filenames[item]
        mask = cv2.imread(mask_path, cv2.IMREAD_GRAYSCALE)
        mask = np.float32(mask / 255)
        mask = mask[None, ...]
        self.iter_i += 1
        return dict(source=input_, target=output_, mask=mask)
...
def make_default_val_dataset(indir, kind='default', out_size=512, transform_variant=None, **kwargs):
